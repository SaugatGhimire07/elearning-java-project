...
public class CourseDao {
	...
	public Course getCourseById(int courseId) {
		String query = "SELECT c.*, u.full_name AS instructor_name, ca.category_name " + "FROM Course c "
				+ "JOIN User u ON c.instructor_id = u.user_id " + "JOIN Category ca ON c.category_id = ca.category_id "
				+ "WHERE c.course_id = ?";
		try (Connection connection = DatabaseConnection.getConnection();
	...
	public List<Course> getAllCourses() {
		...
		String query = "SELECT c.*, u.full_name AS instructor_name " + "FROM Course c "
	...
	public List<Course> getLastFourCourses() {
		...
		String sql = "SELECT c.course_id, c.course_title, c.description, c.price, c.cover_image_url, u.full_name AS instructor_name "
				+ "FROM Course c " + "JOIN User u ON c.instructor_id = u.user_id " + "WHERE u.user_type = 'Instructor' "
	...
	public int countLessons(int courseId) {
		String query = "SELECT COUNT(*) AS lesson_count FROM lessons WHERE course_id = ?";
		...
		try (Connection connection = DatabaseConnection.getConnection();
			...
			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next()) {
					return rs.getInt("lesson_count"); // Return the count of lessons
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return 0; // Return 0 if there is an issue or no lessons found
	}

	// New method to check if a student is enrolled in a course
